...
const BetbryDouble_1 = __importDefault(require("../../Models/BetbryDouble"));
const PadroesBetbryDouble_1 = __importDefault(require("../../Models/PadroesBetbryDouble"));
...
class BetbryDoubleController {
    async BetbryDouble() {
        Ws_1.default.boot();
        const Websocket = require('ws');
        open();
        function open() {
            var SocketBetbryDouble = new Websocket('wss://aws3.rescdns.com/gs2/listen?module=jU86x7bq');
            SocketBetbryDouble.setMaxListeners(20);
            SocketBetbryDouble.onopen = () => {
                console.log('Conectado ao Betbry Double');
                SocketBetbryDouble.send(JSON.stringify({ "data": "switch-module", "modules": ["jU86x7bq"] }));
                setTimeout(() => {
                    SocketBetbryDouble.send(JSON.stringify({ "data": "switch-module", "modules": ["jU86x7bq", "rowDouble", "global"] }));
                }, 500);
            };
            SocketBetbryDouble.onclose = function (e) {
                console.log('Socket betbry Double is closed. Reconnect will be attempted in 1 second.', e.reason);
                setTimeout(function () {
                    open();
                }, 1000);
            };
            SocketBetbryDouble.onerror = function (err) {
                console.error('Socket betbry Double  encountered error: ', err.message, 'Closing socket');
                SocketBetbryDouble.close();
            };
            let ç = [];
            let refund = [];
            SocketBetbryDouble.onmessage = (e) => {
                if (!e) {
                    return;
                }
                else {
                    var i = e.data.split(':');
                    var i2 = (i[34] + i[35] + i[36]);
                    var tp = String(String(i[15]).split(',')[0]).replace('\\', '').replace('"', '');
                    var tp2 = String(tp).replace('"', '').replace('\\', '');
                    var result = String(String(i[34]).split('"')[1]).replace('/\\', '');
                    if (tp2 != 'FINISHED') {
                        return;
                    }
                    var doubleColor = String(String(i[14]).split('"')[1]).replace('\\', '').replace('"', '');
                    var doubleNumber = String(String(i[18]).split('"')[0]).replace('\\', '').replace('"', '').replace(',', '');
        ...
        async function double(doubleColor, doubleNumber) {
            ...
            if (doubleNumber === 0) {
                doubleNumber = '<img src="/assets/images/betbry.a065c12e.png">';
            }
            String(doubleHora);
            String(doubleNumber);
            await Database_1.default.transaction(async (trx) => {
                const newdouble = new BetbryDouble_1.default();
            ...
            var Sugestao = [];
            setTimeout(() => {
                Ws_2.default.io.emit('DoubleBetbry', {
        ...
        async function enviarMsgSugestao(texts) {
            const TOKEN = '5814329910:AAFZDJXzFmJgDIYbUMQyWTujn9dL0vIzXkU';
            const chatId = -1001615549819;
            const text = texts;
        ...
        Ws_2.default.io.on('connection', (SocketBetbryDouble) => {
            SocketBetbryDouble.on('pingBD', (data) => {
                SocketBetbryDouble.emit('pongBD', data);
            });
            SocketBetbryDouble.on('qtJogosBetbryDouble', (data) => {
                getJogos(data);
                async function getJogos(data) {
                    let quantidadeJogos = data;
                    let list = await BetbryDouble_1.default.query().orderBy('id', 'desc').limit(quantidadeJogos);
                    if (quantidadeJogos !== 100) {
                        list = await BetbryDouble_1.default.query().orderBy('id', 'desc').limit(quantidadeJogos);
                    }
                    ...
                    if (WhiteToday.length == 0 && RedToday.length == 0 && BlackToday.length == 0 && DataEstatisticas[0] !== dataHoje2) {
                        ...
                        var whiteToday = await BetbryDouble_1.default.query().where('double_cor', 'white').where('created_at', '>=', dataHoje2 + ' 00:00:00');
                        var redToday = await BetbryDouble_1.default.query().where('double_cor', 'red').where('created_at', '>=', dataHoje2 + ' 00:00:00');
                        var blackToday = await BetbryDouble_1.default.query().where('double_cor', 'black').where('created_at', '>=', dataHoje2 + ' 00:00:00');
            ...
            SocketBetbryDouble.on('betbryDoubleNewPadrao', (data) => {
                let padrao = data.padroes;
                let userId = data.userId;
                getNewPadrao(padrao, userId);
                async function getNewPadrao(padrao, userId) {
                    await PadroesBetbryDouble_1.default.create({ padrao: padrao, user_id: userId });
                }
            });
            SocketBetbryDouble.on('betbryDoubleGetPadraoList', (data) => {
                let userId = data.userId;
                putPadrao(userId);
                async function putPadrao(userId) {
                    let listPadrao = await PadroesBetbryDouble_1.default.query().where('user_id', '=', userId);
                    ...
                    Ws_2.default.io.emit('DoubleBetbry', { listPadrao2 });
                }
            });
            SocketBetbryDouble.once('betbryDoubleDeletePadrao', (data) => {
                let idPadrao = data.idPadrao;
                deletePadrao(idPadrao);
                async function deletePadrao(idPadrao) {
                    await PadroesBetbryDouble_1.default.query().where('id', '=', idPadrao).delete();
                }
            });
            SocketBetbryDouble.on('PadroesShareBetbryDouble', (data) => {
                let idPadrao = data.idPadrao;
                let userId = data.userId;
                let name = data.nameUser;
                let casaDeAposta = data.casaDeAposta;
                let casaDeApostaTipo = data.casaDeApostaTipo;
                BetbryDouble(idPadrao, userId, name, casaDeAposta, casaDeApostaTipo);
                async function BetbryDouble(idPadrao, userId, name, casaDeAposta, casaDeApostaTipo) {
                    let idPadraoBusca = idPadrao - 122900000000;
                    let padraoBusca = await PadroesBetbryDouble_1.default.query().where('id', idPadraoBusca).first();
                ...
                async function salvarPadrao(userId, name, casaDeAposta, casaDeApostaTipo, padrao) {
                    ...
                    if (padraoBusca !== null) {
                        SocketBetbryDouble.emit('PadroesShareBetbryDoubleMsg', { error: 'Padrão já compartilhado' });
                    }
                    else {
                        ...
                        SocketBetbryDouble.emit('PadroesShareBetbryDoubleMsg', { success: 'Padrão compartilhado com sucesso' });
                    }
                }
            });
        });
...
exports.default = BetbryDoubleController;
//# sourceMappingURL=BetbryDoubleController.js.map