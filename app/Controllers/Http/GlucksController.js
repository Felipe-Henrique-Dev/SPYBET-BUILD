...
class GlucksController {
    async gluck({ auth, session, response }) {
        ...
        try {
            ...
            return response.redirect('/gluck/crash/' + userId);
        }
        catch (err) {
    ...
    async crash({ view, auth, response, session }) {
        ...
        try {
            ...
            const userId = Number(auth.user.id) + 122930000000;
            return view.render('gluck-tabela-crash', { userId });
        }
        catch (err) {
            console.log('erro voltou ao login');
            return response.redirect('/auth/login');
    ...
    async double({ view, auth, response, session }) {
        ...
        try {
            ...
            const userId = Number(auth.user.id) + 122930000000;
            return view.render('gluck-tabela-double', { userId });
        }
        catch (err) {
            console.log('erro voltou ao login');
            return response.redirect('/auth/login');
...
exports.default = GlucksController;
//# sourceMappingURL=GlucksController.js.map